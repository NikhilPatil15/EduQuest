...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const profileResponse = await userAPI.getGameProfile();
  ...
  return (
    ...
    >
      ...
      {/* Background layers - same as signup page */}
      <div
        aria-hidden
        data-parallax="-0.4"
        className="absolute inset-0 -z-10 bg-[radial-gradient(60%_50%_at_50%_40%,rgba(255,136,68,0.4),rgba(179,0,0,0.25),transparent_70%),linear-gradient(180deg,#0a0303_0%,#1a0a0a_50%,#2d0f0f_100%)]"
      ></div>
      <div
        aria-hidden
        className="dither-overlay absolute inset-0 -z-10 opacity-70"
      ></div>
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="pixel-grid absolute inset-0 -z-10 opacity-70"
      ></div>
      <div
        aria-hidden
      ...
      >
        {[...Array(50)].map((_, i) => (
          <span
            key={i}
            className="pixel-particle absolute w-1 h-1 bg-blue-400/80"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
              boxShadow: "0 0 6px #4d79ff",
            }}
          ></span>
        ))}
      </div>
      ...
      {/* Login Form */}
      <div className="relative z-10 p-6 md:p-10 bg-black/70 border-8 border-blue-800/80 shadow-[10px_10px_0_#000] max-w-md w-full mx-auto pixelated-rendering">
        {/* Decorative elements with blue theme */}
        <div className="floating-orb absolute -top-8 -left-8 w-12 h-12 bg-blue-500 rounded-full border-4 border-black shadow-[4px_4px_0_#000] flex items-center justify-center">
          <img
            src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/great-ball.png"
            alt="Great Ball"
            className="w-8 h-8 pixelated-rendering"
          />
        </div>
        <div className="floating-orb absolute -bottom-6 -right-6 w-10 h-10 bg-cyan-400 rounded-full border-4 border-black shadow-[4px_4px_0_#000]"></div>
        <img
          ...
          alt="Squirtle"
          className="floating-orb absolute -top-12 right-10 w-24 h-24 pixelated-rendering drop-shadow-[4px_4px_0_#000]"
        />
        <img
          src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/ultra-ball.png"
          alt="Ultra Ball"
          className="floating-orb absolute -bottom-10 left-10 w-16 h-16 pixelated-rendering"
        />
        <div className="absolute -inset-1 border-2 border-blue-400/60 blur-[1px]"></div>

        <h2
          ref={formTitleRef}
          className="text-3xl md:text-4xl font-bold text-center text-shadow-pixel mb-8 bg-blue-800/30 px-4 py-2 rounded"
        >
          WELCOME BACK!
        </h2>
        ...
        <form ref={formRef} className="space-y-6" onSubmit={handleLogin}>
          ...
          <div>
            <label
              htmlFor="password"
              className="block text-blue-200 text-sm mb-2"
            >
              PASSWORD
            </label>
            <input
          ...
          <div className="flex flex-col md:flex-row gap-4 pt-4">
            <PixelButton
              type="submit"
              className="flex-1 w-full text-lg py-3 bg-blue-600 hover:bg-blue-700"
              disabled={isLoading}
            >
            ...
            <PixelButton
              type="button"
              className="flex-1 w-full bg-gray-700 hover:bg-gray-800 !shadow-[6px_6px_0px_#222] text-lg py-3"
