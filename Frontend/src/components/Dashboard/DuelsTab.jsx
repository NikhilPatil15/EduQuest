...
gsap.registerPlugin(TextPlugin); // 👈 Register GSAP TextPlugin
...
// --- HELPER COMPONENTS (Required for Battle UI) ---
>>>>>>> Stashed changes
...
            // Use the GSAP TextPlugin to type out the question
            gsap.to(textRef.current, { 
                duration: text.length * 0.03, 
                text: text, 
                ease: 'none',
                onUpdate: () => setDisplayedText(textRef.current.textContent) // Update React state for rerenders if needed
            });
        }
    }, [text]);
    
    return (
        <div className="bg-black/70 border-4 border-black p-4 shadow-[8px_8px_0_#000] min-h-[100px] pixel-font">
            <p ref={textRef} className="text-xl leading-tight text-white"></p>
        </div>
    );
};
...
        player1: null,
        player2: null,
        player1Pokemon: 'pikachu', // Default player pokemon
        player2Pokemon: 'bulbasaur', // Default opponent pokemon
        currentQuestion: null,
...
        selectedSubject: null,
        answered: false,
        attackAnimation: null, // { attacker: 'player'|'opponent', target: 'player'|'opponent', damage: number, isCorrect: boolean }
        chatMessages: [],
        playerCorrect: 0,
        opponentCorrect: 0,
        totalRounds: 5,
...
        setBattleState(prev => ({
            ...prev,
            battleLog: [`[${new Date().toLocaleTimeString()}] ${message}`, ...prev.battleLog.slice(0, 4)]
        }));
    };
...
        // In a real scenario, emit chat to server
        battleSocket.emit('chat-message', { roomId: battleState.roomId, message, senderId: myId });
    };

    // Timer control (Local UI display only)
    const startTimer = (timeLimit = 15) => {
        if (timerRef.current) clearInterval(timerRef.current);
        setBattleState(prev => ({ ...prev, timer: timeLimit }));
        
        timerRef.current = setInterval(() => {
            setBattleState(prev => {
                if (prev.timer <= 1) {
                    clearInterval(timerRef.current);
                    if (!prev.answered && prev.status === "active" && prev.currentQuestion) {
...
    // --- ANIMATION LOGIC (Integrated from QuizSession) ---

    // Trigger animation when attack data changes
    useEffect(() => {
...
        const attackerRef = attacker === 'player' ? playerPokemonRef : opponentPokemonRef;
        const targetRef = target === 'player' ? playerPokemonRef : opponentPokemonRef;

        if (attackerRef.current && targetRef.current && battleAreaRef.current) {
            const tl = gsap.timeline({
                onStart: () => {
                    // Screen Shake Effect (from QuizSession)
                    gsap.to(".duels-tab-container", { x: 'random(-5, 5)', y: 'random(-5, 5)', duration: 0.1, repeat: 3, yoyo: true, ease: 'power1.inOut', clearProps: 'x,y' });
                },
                onComplete: () => {
                    // Clear animation state after it's done
                    setBattleState((prev) => ({ ...prev, attackAnimation: null }));
                }
            });

            // Attacker Move Forward (Combined from original + QuizSession)
...
            if (isCorrect) {
                // Target Hit Animation (Recoil and Color filter)
                tl.to(targetRef.current, {
                    x: attacker === 'player' ? 15 : -15, 
                    y: -5, 
                    opacity: 0.7, 
                    duration: 0.1, 
                    ease: "power2.in" 
                }, "<") // starts at the same time as attacker pulls back
...
                damageText.className = 'absolute text-3xl font-bold text-yellow-300 text-shadow-pixel pixel-font';
...
                gsap.to(damageText, { y: -50, opacity: 0, duration: 1, ease: 'power2.out', onComplete: () => damageText.remove() });
            }
        }
    };
...
                gsap.to(winnerRef.current, { scale: 1.2, y: -15, duration: 0.5, repeat: 1, yoyo: true, ease: 'bounce.out' });
            }
            if (loserRef.current) {
                gsap.to(loserRef.current, { opacity: 0, y: 20, duration: 1, ease: 'power1.in' });
            }
        }
    };
...
            const opponentData = data.player1._id === myId ? data.player2 : data.player1;
            setBattleState(prev => ({
                ...prev,
                status: 'subject_select',
                roomId: data.roomId,
                player1: data.player1,
                player2: data.player2,
                chatMessages: [{ message: `Match found! Your opponent is ${opponentData.userName}.`, sender: 'system' }]
            }));
        });
        
...
            const isPlayer1Update = battle.player1?._id === myId;
            // FIX: Use optional chaining and nullish coalescing for safe health access
            const myHealth = isPlayer1Update ? (battle.player1Health ?? battleState.playerHealth) : (battle.player2Health ?? battleState.playerHealth);
            const oppHealth = isPlayer1Update ? (battle.player2Health ?? battleState.opponentHealth) : (battle.player1Health ?? battleState.opponentHealth);
            
            const newOpponentPokemon = battle.subject ? subjectPokemon[battle.subject] : battleState.player2Pokemon;
...
                player2Pokemon: newOpponentPokemon,
                answered: battle.currentQuestion?._id !== prev.currentQuestion?._id ? false : prev.answered,
            }));
            
            // Start timer if transitioning to active with a new question
            if (battle.status === 'active' && battle.currentQuestion && battle.currentQuestion._id !== battleState.currentQuestion?._id) {
                startTimer(battle.currentQuestion.timeLimit || 15);
                addBattleLog(`❓ Round ${battle.round || 1} begins!`);
            }
        });

        // 3. Round Results Update
        battleSocket.on('round-results', (data) => {
            clearInterval(timerRef.current);
            setBattleState(prev => ({ ...prev, answered: true, status: 'awaiting_results' }));
...
            const oppResult = isPlayer1 ? data.player2 : data.player1;
            ...
            if (attacker) {
                ...
                const attackerName = attacker === 'player' ? 'You' : opponentName;
                addBattleLog(`⚡ ${attackerName} strikes! Damage: ${Math.round(data.damageDealt)}.`);
                
                // Trigger the animation
                setBattleState(prev => ({
                    ...prev,
                    attackAnimation: { attacker, target, damage: data.damageDealt, isCorrect: true },
                    playerCorrect: prev.playerCorrect + (myResult.isCorrect ? 1 : 0),
                    opponentCorrect: prev.opponentCorrect + (oppResult.isCorrect ? 1 : 0),
                }));
            } else {
                addBattleLog('❌ Both missed or drew! No damage dealt.');
            }

            // Server should automatically send the next 'battle-update' or 'battle-finished' after this.
        });
        
...
            if (timerRef.current) clearInterval(timerRef.current);
        };
    }, [myId, opponentName, isPlayer1]);

    // --- SOCKET.IO ACTIONS (Emit Events) ---

    const findBattle = () => {
        if (!battleState.selectedSubject) return;
        setBattleState(prev => ({ ...prev, status: 'searching' }));
        battleSocket.emit('matchmaking-request', { playerId: myId, subject: battleState.selectedSubject });
        addBattleLog(`🔍 Searching for a ${battleState.selectedSubject.toUpperCase()} battle...`);
    };
...
        const timeSpent = (battleState.currentQuestion.timeLimit || 15) - battleState.timer;
        
        battleSocket.emit('player-answer', {
            roomId: battleState.roomId,
            playerId: myId,
...
        if (timerRef.current) clearInterval(timerRef.current);
        setBattleState({
            roomId: null, player1: null, player2: null, player1Pokemon: 'pikachu', player2Pokemon: 'bulbasaur',
            currentQuestion: null, status: 'idle', winner: null, rewards: null, round: 0,
            playerHealth: 100, opponentHealth: 100, playerReady: false, opponentReady: false, timer: 15,
            battleLog: [], selectedSubject: null, answered: false, attackAnimation: null,
            chatMessages: [], playerCorrect: 0, opponentCorrect: 0, totalRounds: 5,
        });
        addBattleLog('🏠 Left the arena.');
    };
...
                                    className={`cursor-pointer p-4 border-4 transition-all duration-200 pixel-button-shadow bg-gray-800 ${
                                        battleState.selectedSubject === subject ? 'border-yellow-500 ring-4 ring-yellow-300' : 'border-black hover:border-yellow-400'
                                    }`}
                                >
                                    <img src={getPokemonByRef(subjectPokemon[subject]).gif} alt={subject} className="mx-auto w-24 h-24" style={{ imageRendering: 'pixelated' }} />
                                    <P className='mt-2 font-bold text-lg text-white capitalize'>{subject}</P>
...
                            className={`mt-6 ${battleState.selectedSubject ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600'} text-white px-8 py-3 rounded-lg border-4 border-yellow-500 font-bold text-lg transform hover:scale-105 transition-all duration-300 pixel-button-shadow disabled:bg-gray-600 disabled:cursor-not-allowed`}
                        >
                            {`Find ${battleState.selectedSubject ? battleState.selectedSubject.toUpperCase() : ''} Battle!`}
                        </button>
                    </div>
                );
            case 'searching':
                return (
                    <div className="text-center p-12 min-h-72 flex flex-col items-center justify-center">
                        <div className="animate-pulse text-yellow-300 text-xl mb-6 pixel-font">🔍 Searching for opponent...</div>
                        <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png" alt="Searching" className="w-32 h-32 mx-auto mb-6 relative animate-spin" style={{ animationDuration: '2s', imageRendering: 'pixelated' }} />
                        <button onClick={leaveBattle} className="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded border-2 border-black pixel-font">Cancel</button>
                    </div>
                );
            case 'subject_select':
...
                        <H3 className="text-xl font-bold mb-4 text-yellow-200 text-shadow-pixel">MATCH FOUND! {battleState.status === 'ready' ? 'WAITING FOR OPPONENT' : 'GET READY!'}</H3>
                        <H4 className="text-lg font-bold mb-4 text-white text-shadow-pixel">Opponent: {opponentName}</H4>
                        <div className="flex justify-center space-x-8 mb-6">
                            <div className="px-4 py-2 rounded-lg border-2 border-black pixel-font bg-blue-600">You: {battleState.playerReady ? '✅ Ready' : '❌ Not Ready'}</div>
                            <div className="px-4 py-2 rounded-lg border-2 border-black pixel-font bg-red-600">{opponentName}: {battleState.opponentReady ? '✅ Ready' : '❌ Not Ready'}</div>
                        </div>
                        <button onClick={markReady} disabled={battleState.playerReady} className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-bold text-lg transform hover:scale-105 transition-all duration-300 border-4 border-black pixel-button-shadow disabled:bg-gray-500">
                            {battleState.playerReady ? 'Waiting...' : "I'm Ready!"}
                        </button>
                    </div>
                );
            case 'active':
...
                                <div className={`mx-auto w-16 h-16 flex items-center justify-center rounded-full border-4 border-black text-white text-2xl font-bold pixel-font text-shadow-pixel ${battleState.timer <= 5 && battleState.status === 'active' ? 'bg-red-600 animate-pulse' : 'bg-gray-700'}`}>
                                    {battleState.timer}
                                </div>
                            </div>
                            
                            {/* Player Pokémon & UI */}
                            <div id="player-char" className="absolute left-10 bottom-10 transform transition-all duration-300" ref={playerPokemonRef}>
                                <div className="w-40 bg-gray-800 border-4 border-black p-2 absolute -top-16 -left-2 rounded-lg shadow-lg">
                                    <P className="font-bold text-white text-sm pixel-font truncate">{player?.userName || 'Player'}</P>
                                    <div className="w-full bg-gray-600 rounded-full h-3 border border-black"><div className="bg-green-500 h-full rounded-full transition-all duration-500" style={{ width: `${Math.round(battleState.playerHealth)}%` }}></div></div>
                                    <div className="text-xs text-gray-300 pixel-font text-right">{Math.round(battleState.playerHealth)}/100</div>
                                </div>
                                <img 
                                    src={getPokemonByRef(battleState.player1Pokemon).gif} 
                                    alt={battleState.player1Pokemon}
                                    className="w-24 h-24"
...
                            {/* Opponent Pokémon & UI */}
                            <div id="opponent-char" className="absolute right-10 top-10 transform transition-all duration-300" ref={opponentPokemonRef}>
                                <div className="w-40 bg-gray-800 border-4 border-black p-2 absolute -bottom-16 -right-2 rounded-lg shadow-lg">
                                    <P className="font-bold text-white text-sm pixel-font text-right truncate">{opponentName}</P>
                                    <div className="w-full bg-gray-600 rounded-full h-3 border border-black"><div className="bg-green-500 h-full rounded-full transition-all duration-500" style={{ width: `${Math.round(battleState.opponentHealth)}%` }}></div></div>
                                    <div className="text-xs text-gray-300 pixel-font">{Math.round(battleState.opponentHealth)}/100</div>
                                </div>
                                <img 
                                    src={getPokemonByRef(battleState.player2Pokemon).gif} 
                                    alt={battleState.player2Pokemon}
                                    className="w-24 h-24"
                                    style={{ imageRendering: 'pixelated' }}
                                />
                            </div>
                            
                            {/* Awaiting Results Overlay */}
                            {battleState.status === 'awaiting_results' && (
                                <div className="absolute inset-0 flex items-center justify-center bg-black/70 rounded-lg z-20">
                                    <H2 className="text-4xl text-yellow-300 animate-pulse text-shadow-pixel">AWAITING RESULTS...</H2>
                                </div>
                            )}
                        </div>
...
                        {battleState.currentQuestion && (
                            <div className="bg-red-800 border-4 border-black rounded-lg p-4 mb-4 shadow-lg pixel-box">
                                <DialogueBox text={battleState.currentQuestion.question} />
...
            case 'finished':
                const isWinner = battleState.winner === myId;
                const totalQuestions = battleState.playerCorrect + battleState.opponentCorrect;
                const correctRatio = `${battleState.playerCorrect} / ${totalQuestions}`;
                
                return (
                    <div className="text-center p-6 min-h-72">
...
                            <P className="text-lg mb-2">XP Gained: <span className="text-green-400 font-bold">+{battleState.rewards?.xp || (isWinner ? 100 : 25)}</span></P>
                        </div>
...
    return (
        <div className="text-white bg-red-900 min-h-screen p-8 duels-tab-container" style={{ imageRendering: 'pixelated' }}>
            <style>{` 
                /* Add the Press Start 2P font for the pixel style */
                @import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap'); 
                .pixel-font { font-family: 'Press Start 2P', cursive; } 
                .text-shadow-pixel { text-shadow: 4px 4px 0 #000; } 
                .pixel-button-shadow { box-shadow: 6px 6px 0px #000; }
                .pixel-box { box-shadow: 8px 8px 0 #000; } 
                .duels-tab-container { background-image: radial-gradient(circle at 50% 10%, #4a0000 0%, #1a0a0a 100%); }
            `}</style>
