...
const pokemonSprites = {
  player: {
    name: "Pikachu",
    sprite: "https://play.pokemonshowdown.com/sprites/gen5ani-back/pikachu.gif",
  },
  Science: {
    name: "Magneton",
    sprite:
...
const DAMAGE_EXPLOSION_GIF =
  "https://s3.amazonaws.com/files.dorkly.com/assets/dorkly_fireball.gif"; // A pixelated explosion
  ...
  Coding: {
    ...
    Medium: [
      ...
      },
      {
        ...
        answer: "To iterate over a block of code",
      },
      {
    ...
    Hard: [
      {
        ...
        answer: "The algorithm's time/space complexity",
      },
      {
      ...
      {
        question: "Which version control system is the most widely used?",
  ...
  Science: {
    Easy: [
      ...
      },
      {
        ...
        options: ["Electron", "Proton", "Molecule", "Nucleus"],
        answer: "Nucleus",
      },
      {
    ...
    Medium: [
      ...
      },
      {
        ...
        options: ["Richter", "Beaufort", "Kelvin", "pH"],
        answer: "Beaufort",
      },
      {
    ...
    Hard: [
      {
        ...
        options: ["Mycology", "Virology", "Botany", "Geology"],
        answer: "Mycology",
      },
      {
        question:
          "The Heisenberg Uncertainty Principle is related to what field?",
        options: ["Relativity", "Genetics", "Quantum Mechanics", "Acoustics"],
      ...
      },
      {
        ...
        options: [
          "Speed of sound",
          "Speed of light",
          "Speed to exit gravity",
          "Rotational speed",
        ],
        answer: "Speed to exit gravity",
      },
    ],
  },
  Math: {
    ...
    Medium: [
      ...
      },
      {
        ...
        options: ["Quadrillion", "Quintillion", "Sextillion", "Zillion"],
        answer: "Quadrillion",
      },
      {
      ...
      {
        question:
          "A prime number is a number greater than 1 with only two factors: 1 and itself. Is 51 a prime number?",
        options: ["Yes", "No"],
        answer: "No",
      },
    ],
    Hard: [
      {
        question: "In calculus, what is the derivative of x²?",
        options: ["2x", "x³/3", "x", "2"],
        answer: "2x",
      },
      {
      ...
      {
        question: "What does the 'C' represent in Roman numerals?",
...
const HealthBar = ({ currentHp, maxHp, isPlayer = false }) => {
  ...
  return (
    <div
      className={`w-full bg-black/50 border-4 border-black p-1 shadow-[4px_4px_0_#000] ${
  ...
  <div className={`flex flex-col items-center gap-2 relative ${className}`}>
    {showHit && (
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
        <img
          src={DAMAGE_EXPLOSION_GIF}
          alt="Hit"
          className="w-24 h-24 pixelated-rendering"
        />
        <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-pixel text-4xl text-red-500 text-shadow-pixel-white">
          HIT!
        </span>
      </div>
    )}
    <img
      ...
      src={spriteUrl}
      alt={name}
      className="h-32 md:h-48 pixelated-rendering drop-shadow-[4px_4px_0_rgba(0,0,0,0.5)]"
    />
    <div className="w-48">
...
export const QuizProvider = ({ children }) => {
  ...
  const [wrongAnswersCount, setWrongAnswersCount] = useState(0); // ✨ ADD THIS STATE
  const MAX_HP = 100;
  const MAX_WRONG_ANSWERS = 3; // ✨ ADD THIS CONSTANT

  useEffect(() => {
    if (gameState !== "in_progress") return;
    // 🔄 UPDATED a game over condition
    const isGameOver =
      playerHp <= 0 ||
      opponentHp <= 0 ||
      currentQuestionIndex >= questions.length ||
      wrongAnswersCount >= MAX_WRONG_ANSWERS;
    ...
    gameState,
    wrongAnswersCount,
  ]); // ✨ ADD wrongAnswersCount here

  const startQuiz = (newSettings) => {
    setSettings(newSettings);
    const fetchedQuestions =
      quizData[newSettings.subject]?.[newSettings.difficulty] || [];
    setQuestions(fetchedQuestions);
    setCurrentQuestionIndex(0);
    setScore(0);
    setPlayerHp(MAX_HP);
    setOpponentHp(MAX_HP);
    setWrongAnswersCount(0); // ✨ RESET WRONG ANSWERS
  ...
  const answerQuestion = (isCorrect) => {
    ...
    if (isCorrect) {
      ...
      setWrongAnswersCount((prev) => prev + 1); // ✨ INCREMENT WRONG ANSWERS
    }
...
function QuizSelection() {
  ...
  return (
    <div className="animate-fade-in text-center">
      <h2 className="text-3xl md:text-5xl font-bold text-shadow-pixel mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {Object.keys(quizData).map((subject) => (
          <div
            key={subject}
            onClick={() => handleUpdate("subject", subject)}
            className={`bg-black/40 border-4 p-6 shadow-[8px_8px_0_#000] cursor-pointer transition-all duration-200 hover:scale-105 hover:border-yellow-400 ${
              selection.subject === subject
                ? "border-yellow-400 scale-105 shadow-[8px_8px_0_#eab308]"
                : "border-black"
            }`}
          >
            <img
              src={pokemonSprites[subject].sprite}
              alt={pokemonSprites[subject].name}
              className="h-32 mx-auto pixelated-rendering"
            />
      ...
      <div className="mt-12">
        <h3 className="text-2xl font-bold text-shadow-pixel mb-4">
          SELECT DIFFICULTY
        </h3>
        <div className="flex justify-center gap-4">
          {["Easy", "Medium", "Hard"].map((level) => (
            <button
              ...
              className={`border-4 text-lg border-black px-6 py-3 font-bold shadow-[4px_4px_0_#000] transition-all duration-150 ${
                selection.difficulty === level
                  ? "bg-yellow-400 text-black"
                  : "bg-black/40 hover:bg-red-900/50"
              }`}
            >
              {level}
            </button>
      ...
      <div className="mt-12">
        <PixelButton onClick={() => startQuiz(selection)}>
          Start Battle!
        </PixelButton>
      </div>
    </div>
...
// ✨ (Optional but Recommended) NEW COMPONENT FOR UI
const StrikesIndicator = () => {
    const { wrongAnswersCount, MAX_WRONG_ANSWERS } = useQuiz();
    return (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 flex gap-2 items-center bg-black/50 px-3 py-1 border-2 border-black rounded-full shadow-md">
            <span className="text-sm font-bold text-white pr-2">Strikes:</span>
            {Array.from({ length: MAX_WRONG_ANSWERS }).map((_, i) => (
                <span key={i} className={`w-6 h-6 flex items-center justify-center font-bold text-xl rounded-full border-2 border-black ${i < wrongAnswersCount ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                    X
                </span>
            ))}
        </div>
...
function QuizSession() {
    // 🔄 Destructure wrongAnswersCount and MAX_WRONG_ANSWERS from context
    ...
    // ... (keep the existing handleAnswer, useEffect hooks as they are) ...
        ...
        <div className="animate-fade-in flex flex-col h-[75vh]">
            <div className="flex-grow flex justify-between items-center px-4 relative">
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2">
                     {/* TIMER UI */}
                    <div className={`w-20 h-20 flex items-center justify-center font-bold text-4xl border-4 border-black bg-black/50 rounded-full shadow-[4px_4px_0_#000] transition-colors duration-300 ${timer <= 5 ? 'text-red-500' : 'text-yellow-300'}`}>
                        {timer}
                    </div>
                </div>
                {/* ✨ ADD THE STRIKES INDICATOR HERE */}
                <StrikesIndicator />
            ...
            {/* ... (keep the rest of the QuizSession component the same) ... */}
...
function QuizResults() {
    ...
    const { score, questions, resetQuiz, playerHp, opponentHp, settings, wrongAnswersCount, MAX_WRONG_ANSWERS } = useQuiz();
    
    // 🔄 MORE ROBUST victory condition
    const isVictory = opponentHp <= 0 && playerHp > 0;
    const lostByStrikes = wrongAnswersCount >= MAX_WRONG_ANSWERS;

    return (
        <div className="animate-fade-in text-center">
            {isVictory ? ( 
                <div>
                    <h2 className="text-4xl md:text-6xl font-bold text-shadow-pixel text-yellow-300 mb-4">YOU WON!</h2>
                    <p className="text-xl mb-6">Your knowledge was super effective!</p>
                </div>
            ) : ( 
                <div>
                    <h2 className="text-4xl md:text-6xl font-bold text-shadow-pixel text-red-500 mb-4">DEFEATED...</h2>
                    {/* ✨ ADD conditional loss message */}
                    {lostByStrikes ? (
                         <p className="text-xl mb-6">Too many incorrect answers! Your opponent overwhelmed you.</p>
                    ) : (
                         <p className="text-xl mb-6">Time to hit the books and train harder!</p>
                    )}
                </div>
            )}
  ...
  return (
    <QuizProvider>
      <div className="min-h-screen text-white overflow-hidden relative font-pixel bg-[#1a0a0a] quiz-page-container">
        ...
        <div
          aria-hidden
          className="absolute inset-0 -z-10 bg-[radial-gradient(60%_50%_at_50%_40%,rgba(255,136,68,0.2),rgba(179,0,0,0.15),transparent_80%)]"
        ></div>
        <div
          aria-hidden
          className="absolute inset-0 -z-10"
          style={{ boxShadow: "inset 0 0 180px 40px rgba(0,0,0,0.75)" }}
        ></div>
        <header className="bg-black/50 border-b-4 border-red-600 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
